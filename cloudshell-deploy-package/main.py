...
def extract_agent_type(path, event):
    ...
    # FIRST: Check body for explicit agent parameter (highest priority)
    body_str = event.get('body', '{}')
    if body_str and body_str != 'null':
        try:
            body = json.loads(body_str)
            if isinstance(body, dict) and 'agent' in body:
                agent_param = body['agent'].lower()
                ...
                elif agent_param == 'chat':
                    return 'SM-Asst-AgileCoachingAgent'  # Default chat to coaching
        except:
            pass
...
def get_backlog_response(message: str, message_lower: str) -> str:
    ...
    # Check if this is specifically about shopping cart
    is_shopping_cart = any(keyword in message_lower for keyword in ['shopping cart', 'cart', 'e-commerce', 'ecommerce', 'online store', 'add to cart', 'checkout'])
    ...
    if any(keyword in message_lower for keyword in ['user story', 'user stories', 'story', 'stories']) or is_shopping_cart or is_login_app:
        if is_shopping_cart:
            return f"""**📋 Backlog Intelligence Agent - E-commerce Shopping Cart User Stories**
...
- When I change the quantity of an item (increase/decrease)
- Then the item subtotal updates immediately
- And the cart total recalculates automatically
- And I can remove items by setting quantity to zero
...
- And I see a confirmation of the removal
...
- And prices are updated to current values

Story Points: 8
```

```
Story 6: Proceed to Checkout
As a customer
...
- Then I am taken to the checkout flow
- And my cart contents are carried forward
- And I cannot checkout with an empty cart

Story Points: 3
```

```
Story 7: Cart Total Calculation
As a customer
I want to see accurate pricing and tax calculations
So that I know exactly what I'll pay

Acceptance Criteria:
- Given I have items in my cart
- When I view the cart summary
- Then I see subtotal, tax amount, shipping costs, and grand total
- And calculations update when I modify quantities
- And taxes are calculated based on my location

Story Points: 5
```

```
Story 8: Stock Availability Check
As a customer
I want to know if cart items are still available
So that I'm not disappointed at checkout

Acceptance Criteria:
- Given I have items in my cart
- When product stock levels change
- Then I'm notified if items become unavailable
- And I can remove out-of-stock items
- And quantities auto-adjust if requesting more than available

Story Points: 8
```

**Epic Breakdown:**
- **Basic Cart Operations** (Stories 1-4): Core add/remove/update functionality
- **Enhanced Experience** (Stories 5-6): Persistence and checkout integration  
- **Business Logic** (Stories 7-8): Pricing calculations and inventory management

**Additional Advanced Stories:**
- Save cart for later / Wishlist
- Cart abandonment email reminders
- Bulk quantity discounts
- Gift wrapping options
- Express checkout (one-click purchase)
- Cart sharing with others
- Mobile-optimized cart experience

**Acceptance Criteria Best Practices:**
- Always include error handling scenarios
- Test with edge cases (empty cart, maximum quantities)
- Verify mobile responsiveness
- Include accessibility requirements
- Test performance with large carts (50+ items)

Would you like me to elaborate on any specific story or create additional user stories for related features?"""
