...
# Deploy enhanced SM Assistant with better responses to Lambda
set -e

echo "🚀 Deploying enhanced SM Assistant with intelligent responses..."

# Create enhanced package
mkdir -p lambda-enhanced
cd lambda-enhanced

# Create enhanced Lambda handler with better AI-like responses
cat > main.py << 'EOF'
"""
...
echo "📤 Deploying enhanced Lambda..."
...
echo "✅ Enhanced SM Assistant deployed!"
echo ""
echo "🎯 Testing enhanced version..."
sleep 15

echo "Testing health endpoint:"
curl -s "https://$API_ID.execute-api.us-east-1.amazonaws.com/prod/health" | jq '.enhanced_mode'

echo ""
echo "Testing Shadowdark RPG user stories:"
curl -X POST "https://$API_ID.execute-api.us-east-1.amazonaws.com/prod/agents/backlog" \
  -H "Content-Type: application/json" \
  -d '{"message": "Write user stories for a Shadowdark RPG game master assistant agent. It helps GMs run their campaign."}' \
  -s | jq -r '.message'
EOF